...
* 官方文档参考：[cuDNN Graph API](https://docs.nvidia.com/deeplearning/cudnn/latest/developer/graph-api.html)
* 算子融合的灵活性是大模型时代最核心的诉求。为了支持多样且灵活的融合模式，并覆盖各种复杂的应用场景，cuDNN 引入了运行时融合引擎（即具有 `CUDNN_BEHAVIOR_NOTE_RUNTIME_COMPILATION` 行为属性的引擎），它会根据提交的计算图拓扑结构在运行期实时生成并编译 Kernel。

下图展示了运行时融合引擎支持的部分算子融合子图模式：

![](../assets/knlfusion1.png)
![](../assets/knlfusion2.png)

*(注：你需要检查你所使用的 GPU 的计算兼容性以确定哪些具体操作能够被硬件支持成功融合)*

cuDNN 将其 API 划分为以下四大功能模块：
1. **Graph API (图 API)**：实现算子融合（Kernel Fusion）。图中的节点（Nodes）代表计算操作，边（Edges）代表张量数据流。
2. **Ops API (算子 API)**：实现单一独立运算（如 Softmax、BatchNorm、Dropout 等）。
3. **CNN API (卷积神经网络 API)**：实现经典的卷积计算及相关操作（由 Graph API 隐式调用）。
4. **Adversarial API (其他 API)**：实现其他特定的算法与网络层（如 RNNs、CTC 损失函数、Multi-Head Attention 多头注意力等）。
