...
- [Q6: cuDNN Graph API 是如何通过运行时即时编译 (JIT) 实现算子融合并让数据在寄存器中流转的？](#q6-cudnn-graph-api-是如何通过运行时即时编译-jit-实现算子融合并让数据在寄存器中流转的)
- [Q7: 什么是 NCHW 与 NHWC 数据排布格式？为什么在 GPU 性能调优中更推荐使用 NHWC (Channels-Last)？](#q7-什么是-nchw-与-nhwc-数据排布格式为什么在-gpu-性能调优中更推荐使用-nhwc-channels-last)
...
### Q6: cuDNN Graph API 是如何通过运行时即时编译 (JIT) 实现算子融合并让数据在寄存器中流转的？
**A**:

cuDNN Graph API 能够实现运行时的算子融合与极致加速，主要依赖于**动态代码拼接**、**NVRTC 运行时编译**以及 **GPU 寄存器硬件分配**的组合机制。其底层的完整工作流水线如下：

#### 1. 构建计算图的有向无环图 (DAG)
在主机端（CPU），开发者调用 Graph API 构建计算图，声明所有的虚拟 Tensor 节点与数学计算 Node（如卷积、加法、激活等）。这在 cuDNN 内部形成了一个**抽象计算图（DAG）**的逻辑树结构。

#### 2. 动态拼接 CUDA C++ 代码字符串
一旦计算图被 finalize，cuDNN 的 JIT 编译器便会遍历该 DAG。如果发现子图结构符合融合引擎模板（即带有 `CUDNN_BEHAVIOR_NOTE_RUNTIME_COMPILATION` 属性的引擎），它就会在 CPU 内存中**动态拼接出一段符合当前图拓扑结构的 CUDA C++ 源代码字符串**。
例如，针对 `Y = ReLU(Conv(X) + Bias)` 这一串联路径，cuDNN 拼接生成的 GPU 核函数逻辑大致如下：
```cpp
...
#### 3. 调用 NVRTC 进行运行时编译
代码拼接完成后，cuDNN 底层加载 **NVRTC** (NVIDIA Runtime Compilation) 库执行即时编译：
* **PTX 编译**：NVRTC 将刚才的 CUDA C++ 代码字符串直接在内存中编译为中间虚拟汇编指令 **PTX**。
* **SASS 翻译**：驱动程序进一步将 PTX 优化翻译为针对当前 GPU 物理架构（如您的 Ada Lovelace 微架构 `sm_89`）的物理机器码 **SASS**。
* **动态加载**：生成的目标机器码通过驱动程序的 `cuModuleLoadData` 以 CUDA 模块形式动态载入 GPU 内存。

#### 4. GPU 寄存器文件 (Register File) 硬件分配
这是避免显存读写瓶颈的硬件秘诀：
* **局部变量寄存器映射**：在生成的核函数代码中，中间结果 `reg_val` 是普通的局部变量。编译成物理机器码时，GPU 编译器会自动将其分配给该线程专属的**物理寄存器**。
* **极限硬件带宽**：GPU 的物理寄存器读写延迟只有 1 个时钟周期，带宽达数十 TB/s；而全局显存（VRAM）延迟高达数百周期，带宽仅数百 GB/s。
* **规避数据搬运**：由于整个流程（卷积 -> 加偏置 -> 激活）在同一个物理线程内线性流转，数据不需要在多线程间共享。因此，它不需要写回 VRAM，甚至不需要走中速的共享内存，全程在超高速的寄存器中流转，彻底消除了显存带宽开销。

#### 5. 绑定物理指针并执行 (Execute)
最后，cuDNN 将你通过 `Variant Pack` 绑定的真实物理显存地址（输入、输出指针）作为参数传递给 JIT 编译好的核函数，并通过 `cudaLaunchKernel` 驱动核函数执行，从而在一趟数据读取中完成了原本需要三个 Kernel 才能算完的任务。

---

### Q7: 什么是 NCHW 与 NHWC 数据排布格式？为什么在 GPU 性能调优中更推荐使用 NHWC (Channels-Last)？
**A**:
...
#### 3. 为什么 GPU/Tensor Cores 加速高度推荐使用 NHWC？
在 NVIDIA GPU 物理硬件（尤其是 **Tensor Cores**）和高性能计算库（如 **cuDNN**）上，采用 **NHWC** 格式往往比 `NCHW` 带来 **20% ~ 100% 以上的吞吐量提升**，这主要取决于以下两个因素：
...
* **物理转置开销**：如果你把 `NCHW` 格式的张量送入 cuDNN 执行 FP16 卷积，cuDNN 为了能够调用 Tensor Cores 硬件，往往被迫在内部先分配临时内存执行一次物理上的**转置操作**（`NCHW -> NHWC`），算完之后再逆向转置回去（`NHWC -> NCHW`）。这多出来的两次全量内存拷贝带来了高昂的显存带宽浪费。直接使用 **NHWC** 能够完全绕过这种转置开销。
...
#### 1. cuBLASmp (多卡单节点矩阵乘法库)
* **定位**：高精度、多进程、GPU 加速的分布式密集线性代数库，主要用于**单机多卡 (Single Node, Multi-GPU)** 环境下的矩阵乘法 (GEMM) 加速。
* **工作机制**：通过多进程机制，自动将一个超出单卡显存容量的超大矩阵切分并分发到单节点内的多张显卡（如 8x H100）上进行协同计算。
* **应用场景**：当单个模型权重或激活值由于尺寸过大而无法放进单张 GPU 显存时，需要使用 cuBLASmp 实现**模型并行 / 张量并行 (Tensor Parallelism)**。
...
* **与 cuBLASmp 的分工**：
  * **cuBLASmp** 专注在单节点内部的多张 GPU 上做实际的矩阵乘加数学计算。
  * **NCCL** 则负责在更大的网络集群尺度上（跨节点、多机）跑批处理、搬运和同步梯度或激活值。
