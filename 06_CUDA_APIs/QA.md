...
#### 1. 核心优势与功能
* **算子融合 / 后处理（Epilogue Fusion）**：这是 cuBLASLt 最强大的功能之一。它允许在矩阵乘法（$C = \alpha AB + \beta C$）完成后，在同一个 GPU Kernel 内直接融合后续计算（例如加上偏置 Bias、应用 ReLU/GELU 等激活函数）。这避免了将中间结果写回显存再读取的开销，极大地节省了显存带宽。
